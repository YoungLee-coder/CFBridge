...
单管理员、多项目的网关：把 Cloudflare **KV**（经 Upstash 风格 **Redis REST**）和 **D1** 以稳定 HTTP API 暴露出去，任意云上的应用都能用边缘存储，不必自己跑在 Workers 上。
...
- **Meta D1**：存项目、资源映射、哈希后的 Key
- **DATA_KV**：共享 KV Binding；新建 KV 资源默认走 Binding（按 `{ref}/` 前缀隔离），极致读性能
- **Dashboard**：Vite React 控制台，打包为 Worker Static Assets，与 API 同源

**Redis / KV：** 默认经 Worker **DATA_KV** Binding（快路径）。「挂载已有 namespace」仍走 Cloudflare Account REST（兼容慢路径）。  
**D1：** 仍经 Account REST（动态按项目选库；无动态 D1 Binding）。

API 与 Dashboard 部署在同一个 Worker（`cfbridge`）里：`/admin` 和 `/v1` 进 Hono，其余走 SPA。
...
本地开发用 `wrangler.dev.toml`（含占位 `META` + `DATA_KV`）；生产部署用 `wrangler.toml`（不写资源 id，靠面板绑 `META` / `DATA_KV`）。
...
| POST | `/admin/projects/:id/resources/create` | 创建资源：KV → 共享 `DATA_KV` Binding；D1 → 经 CF API 新建库 |
...
**不支持**：跨请求的交互式会话（`baton` 恒为 `null`）。需要原子性时，把语句放进同一次 `batch` / pipeline，或在 SQL 里显式 `BEGIN`/`COMMIT`。
...
`wrangler.toml` **故意不写** Meta D1 / DATA_KV 的资源 id，避免覆盖你在面板里的绑定。本地开发请用 `wrangler.dev.toml`（`pnpm dev` / `pnpm db:migrate:local` 已自动指定）。
...
4. 同页再 Add → **KV namespace**，Variable name 必须是 **`DATA_KV`**，选择或创建 `cfbridge-data` → Save  
   （绑定立即生效，**不必**为绑资源再部署一次；没有 `DATA_KV` 时无法「创建」快路径 KV，仍可挂载已有 namespace）
...
2. 确认已绑定 `DATA_KV`（新建 Redis/KV 资源需要）
3. 点 **重新检测**，确认已绑定
4. 点击 **一键初始化 / 升级数据库**
