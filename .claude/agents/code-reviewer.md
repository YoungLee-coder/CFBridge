...
2. **P1 — 约定违反**：未用 `jsonError()` 等 helper；接口类型只改 API/Web 未改 `packages/shared`；Web 新文案只加一种语言；`wrangler.toml` 写入生产 `database_id`；管理路由未挂 `requireReady`。
3. **P2 — 验证缺失**：改动 API/shared/web 后未运行 `pnpm typecheck`；数据面/迁移改动无 curl 或 Setup 验证说明。
